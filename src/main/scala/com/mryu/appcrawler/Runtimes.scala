...
object Runtimes extends CommonLog {
  ...
  def eval(code: String): Unit = {
    if (isLoaded == false) {
      ...
      instance.eval("val driver=com.mryu.appcrawler.AppCrawler.crawler.driver")
      instance.eval("def crawl(depth:Int)=com.mryu.appcrawler.AppCrawler.crawler.crawl(depth)")
      isLoaded = true
    }
    log.info(code)
    instance.eval(code)
