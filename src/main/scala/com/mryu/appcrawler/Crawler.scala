...
import com.mryu.appcrawler.driver.{AppiumClient, MacacaDriver, ReactWebDriver}
import com.mryu.appcrawler.plugin.Plugin
...
class Crawler extends CommonLog {
  ...
  def loadPlugins(): Unit = {
    val defaultPlugins = List(
      "com.mryu.appcrawler.plugin.TagLimitPlugin",
      "com.mryu.appcrawler.plugin.ReportPlugin",
      "com.mryu.appcrawler.plugin.FreeMind"
    )
...
/*    conf.pluginList.foreach(name => {
      if (defaultPlugins.forall(_ != name)) {
        log.info(s"load com.mryu.appcrawler.plugin $name")
